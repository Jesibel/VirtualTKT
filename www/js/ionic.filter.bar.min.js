...
(function(angular, ionic) {
    ...
    angular.module('jett.ionic.filter.bar')
        .factory('$ionicFilterBar', [
            ...
            '$httpParamSerializer',
            '$q',
            function($document, $rootScope, $compile, $timeout, $filter, $ionicPlatform, $ionicFilterBarConfig, $ionicConfig, $ionicModal, $ionicScrollDelegate, $http, $httpParamSerializer, $q) {
                var isShown = false;
                ...
                function filterBar(opts) {
                    ...
                    scope.focusInput = function() {
                        isKeyboardShown = false;
                        //Added option to delay keyboard show due to laggy animation.
                        $timeout(function(){
                            showKeyboard();
                        }, 300);
                    };
                    ...
                    scope.filterItems = function(filterText) {
                        if (!filterText.length) {
							...
							// V2 buscada por ajax
							// pero delegando la logica en un servicio externo
							//Carlos 30S2016
							filteredItems = scope.search(filterText);
							// convertir filteredItems en un promise 
                            ...
                            if (scope.ajax!=null) {
                                if (typeof scope.currentPetition !='undefined'){
                                    scope.currentPetition.promise.status = 499;
                                    scope.currentPetition.resolve("user cancelled");
                                    delete scope.currentPetition;
                                }
                                scope.currentPetition = $q.defer();
                                var _qry = angular.copy(scope.ajax.params);
                                _qry.query = filterText;
                                $http({
                                    method: 'POST',
                                    url: scope.ajax.url,
                                    timeout: scope.currentPetition.promise,
                                    data: $httpParamSerializer(_qry),
                                    headers: {
